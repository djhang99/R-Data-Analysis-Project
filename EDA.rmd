...
# iterate this by each year to pull each suicide100k into a seperate column
# need to change the column name by year and the dataset by year
test1= test1 %>%
...
#low correlation, might be a fit for linear regression
correlationsubset <- corr[c("total_suicides", "gdp_per_capita")]
cor(correlationsubset, use="complete.obs") #looks independent

hist(corr$gdp_per_capita)
hist(corr$total_suicides)

model1 = lm(total_suicides~gdp_per_capita, data=corr)
summary(model1)
...
#an attempt at ANOVA
anova1 = aov(yearly_suicides~age, data=df2)
summary(anova1)
```
